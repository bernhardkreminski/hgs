#!/usr/bin/env node
// .github/scripts/smtp.mjs
//
// A minimal SMTP submission client: connect, authenticate, send one message,
// disconnect. Used by .github/scripts/notify-changes.mjs.
//
// Taken from the sibling project rosencri.me (scripts/smtp.mjs), unchanged
// apart from this header. Keep the two in sync by copying, not by diverging.
//
// Zero npm dependencies — node:net, node:tls, node:crypto only. Nodemailer
// would be one `npm i` away, but this repo has no package.json and no build
// step anywhere, and a notification mailer is not a good enough reason to
// introduce one. It also keeps a third-party Action out of the workflow: no
// credentials handed to code nobody here has read.
//
