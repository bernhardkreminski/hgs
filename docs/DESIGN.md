...
Blogeinträge sind einzeln teilbar (`shareable` in board.js): 🔗-Button in der Aktionsleiste der Karte, Link ist `/blog/#<id>` (die Karte trägt ihre id im DOM). Am Handy öffnet `navigator.share` das System-Menü, sonst geht der Link in die Zwischenablage; klappt beides nicht, zeigt ein Modal den Link. Angesprungene Karte: `.entry-card.is-linked` (kurzes Aufleuchten, Farbe nur über `--accent`) plus `scroll-margin-top`, damit sie nicht unter der Nav klebt. Open-Graph-Tags für die Vorschau stehen im Head von `blog/index.html` — pro Eintrag geht das nicht, der `#`-Anker erreicht den Server nie.
